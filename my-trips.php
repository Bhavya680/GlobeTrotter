...
$allQ = $pdo->prepare("SELECT id, start_date, end_date, status FROM trips WHERE user_id = ?");
$allQ->execute([$userId]);
foreach ($allQ->fetchAll() as $t) {
    $calc = getTripStatus($t['start_date'], $t['end_date']);
    if ($calc !== $t['status']) {
        $pdo->prepare("UPDATE trips SET status = ? WHERE id = ?")
            ->execute([$calc, $t['id']]);
    }
}
...
$tripsQ = $pdo->prepare("
    SELECT t.*,
           ...
           STRING_AGG(DISTINCT c.name, ', ' ORDER BY c.name) AS cities_list,
...
// Total days traveled = sum of durations of completed trips
$totalDays = 0;
foreach ($tripsByStatus['completed'] as $t) {
    if ($t['start_date'] && $t['end_date']) {
        $totalDays += calculateTripDuration($t['start_date'], $t['end_date']);
    }
}
